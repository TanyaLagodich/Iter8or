...
describe('createMapIterator', () => {
  describe('works with sync iterators', () => {
    ...
    it('should work with different types of iterator', () => {
      const string = 'hello';
      const mapIterator = createMapIterator(string, (item) =>
        item.toUpperCase()
      );
    ...
    it('should throw an error, when the method gets a non-iterable argument', () => {
      const nonIterable = 1;
      const mapIterator = () =>
        createMapIterator(nonIterable, (item) => item * 2);
  ...
  describe('works with async iterators', () => {
    ...
    it('should apply a function to each element', async () => {
      ...
      expect(await mapIterator.next()).toEqual({
        done: true,
        value: undefined,
      });
    });
    ...
    it('should work with empty array', async () => {
      ...
      const mapIterator = createMapIterator(
        asyncEmptyIterator,
        (item) => item * 2
      );
